...
class GoodsReceipt extends StatefulWidget {
  const GoodsReceipt();
  @override
  State<GoodsReceipt> createState() => GoodsReceiptState();
}
class GoodsReceiptState extends State<GoodsReceipt>{
  Product? product;
  Warehouse? warehouse;
  ...
  Widget build(BuildContext context) {
      return Scaffold(
        appBar: AppBar(
          title: Text("Goods Receipt",style: TextStyle(fontWeight: FontWeight.bold),),
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Tạo phiếu nhập thành công!')),
          );
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Tạo phiếu nhập thất bại: $errorMessage')),
          );
        }
      } catch (e) {
